...
<template>
  <div class="msg-page">
    <div class="hero-section"><h1 class="hero-title">消息</h1><p class="hero-sub">系统通知 · 私信</p></div>

    <!-- 系统通知 -->
    <div class="section-card">
      <div class="section-header">系统通知</div>
      <div v-if="systemMsgs.length === 0" class="empty-hint">暂无系统通知</div>
      <div v-for="m in systemMsgs" :key="m.id" class="msg-item" :class="{ unread: m.isRead === 0 }" @click="markRead(m.id); if(m.relatedPostId) goPost(m.relatedPostId)">
...
.hero-section { padding: 24px 28px; margin-bottom: 16px; background: #5a67d8; border-radius: 12px; color: #fff; }
.hero-title { font-size: 22px; font-weight: 700; margin: 0 0 4px; }
.hero-sub { margin: 0; opacity: 0.85; font-size: 13px; }
